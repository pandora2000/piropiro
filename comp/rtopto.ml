...
let rec solve_each_element iand_ofs and_group dirvec =
  ...
  if iobj = -1 then ()
    ...
    if t0 <> 0 then
      (
	(*TODO:*)
(*    if iobj = 16 then print_int 0 else ();*)
...
let rec trace_ray nref energy dirvec pixel dist =
  if nref <= 4 then (
	...
	veccpy nvectors.(nref) nvector;
      ...
      (*TODO:追加 moriyatetsuri*)
      (*if obj_id = 16 then print_int 0 else ();*)
...
let rec write_rgb_element x =
  let elem = if x > 255.0 then 255.0 else if x < 0.0 then 0.0 else x in
  let elem = floor elem in
    (*CPU側で出力変更*)
    print_float elem
in
