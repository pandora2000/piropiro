...
let rec trace_reflections index diffuse hilight_scale dirvec =
  ...
  if index >= 0 then (
    ...
    if judge_intersection_fast dvec then
      let surface_id = (int_of_float ((float_of_int intersected_object_id.(0)) *. (float_of_int 4))) + intsec_rectside.(0) in
...
let rec trace_ray nref energy dirvec pixel dist =
  if nref <= 4 then (
    ...
    (* オブジェクトにぶつかった場合 *)
      ...
      (* pixel tupleに情報を格納する *)
      surface_ids.(nref) <- (int_of_float ((float_of_int obj_id) *. (float_of_int 4))) + intsec_rectside.(0);
...
let rec setup_rect_reflection obj_id obj =
  let sid = int_of_float ((float_of_int obj_id) *. (float_of_int 4)) in
...
let rec setup_surface_reflection obj_id obj =
  let sid = (int_of_float ((float_of_int obj_id) *. (float_of_int 4))) + 1 in
  let nr = n_reflections.(0) in
  let br = 1.0 -. o_diffuse obj in
...
(
 image_size.(0) <- size_x;
 image_size.(1) <- size_y;
 image_center.(0) <- int_of_float ((float_of_int size_x) /. (float_of_int 2));
 image_center.(1) <- int_of_float ((float_of_int size_y) /. (float_of_int 2));
