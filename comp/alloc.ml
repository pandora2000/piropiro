...
let replace ((s, g), ienv) gr fgr env fenv tenv =
  let env = map (fun (x, i) -> (x, gr.(i))) env in
  let fenv = map (fun (x, i) -> (x, fgr.(i))) fenv in
    printf "e%!";
  let q x =
    try
      if M.find x tenv = Type.Float then fregs.(assoc x fenv) else regs.(assoc x env)
	(*TODO:*)
  ...
  let p e = match e with
    ...
    | IfEq (x, y, z, u, v, w) -> IfEq (x, q y, q z, u, v, w)
    | IfLE (x, y, z, u, v, w) -> IfLE (x, q y, q z, u, v, w)
    | IfFEq (x, y, z, u, v, w) -> IfFEq (x, q y, q z, u, v, w)
    | IfFLE (x, y, z, u, v, w) -> IfFLE (x, q y, q z, u, v, w)
    | Var (x, y) -> Var (q x, q y)
...
let rec alloc_func ((bid, rids, aids, blks) as f) tenv =
  ...
  let (lout, flout) =
	...
	ft.(i) <- s (fun x -> M.find x tenv = Type.Float) lout.(i) fenv
      done; (Array.to_list t, Array.to_list ft) in
  ...
  let (g, fg) =
    ...
    printf "b%!";
  let (s, fs) =
    let g_empty g = Array.fold_left (fun a b -> b = None && a) true g in
    let d g k l =
